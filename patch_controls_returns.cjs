...
content = content.replace(/\{returnsData\.map\(\(r\) => \(/g, '{paginatedData.map((r) => (');
...
const mobileEndAnchor = `<div className="p-6 text-center text-gray-500">No returns recorded yet.</div>`;
...
console.log('Returns updated for Table Controls.');
